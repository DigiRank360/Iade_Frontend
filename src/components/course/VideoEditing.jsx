...
import {
  ...
  ChevronDown, TrendingUp, Sparkles, Film,
...
const VideoEditing = () => {
  ...
  const TABS = [
    {
      name: "Premiere Pro",
      blocks: [
        { icon: Scissors, label: "Cut Sequence" },
        { icon: Layers, label: "Transition Layer" },
        { icon: Film, label: "Export Timeline" }
      ]
    },
    {
      name: "After Effects",
      blocks: [
        { icon: Sparkles, label: "Text Animation" },
        { icon: Wand2, label: "Motion Graphic" },
        { icon: Layers, label: "Composite Layer" }
      ]
    },
    {
      name: "Color Grading",
      blocks: [
        { icon: Palette, label: "Color Wheel" },
        { icon: Wand2, label: "LUT Preview" },
        { icon: Sparkles, label: "Grade Export" }
      ]
    }
  ];
      ...
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            ...
            if (entry.target.id === 'hero-builder') setTimeout(() => setExported(true), 900);
          }
        });
      },
      { threshold: 0.2 }
  ...
  useEffect(() => {
    if (!isVisible) return;
    const targets = { students: 320, placement: 85, projects: 60 };
    const duration = 2400;
  ...
  const METRICS = [
    ...
    { label: "Video Projects", value: counts.projects, suffix: "+", icon: Clapperboard }
  ];

  const TOOLS = [
    "Premiere Pro", "After Effects", "DaVinci Resolve", "Audition", "Photoshop",
    "Media Encoder", "Color Grading", "Motion Graphics", "Sound Design", "Green Screen"
  ];

  const WHY_CHOOSE = [
    { icon: Laptop, title: "Hands-on Projects", desc: "Work on real-time video projects and create a professional showreel.", metric: "Live" },
    { icon: Target, title: "Live Mentorship", desc: "Learn directly from experienced video editors and media professionals.", metric: "1-on-1" },
    { icon: Shield, title: "Placement Support", desc: "Resume creation, interview preparation & job assistance.", metric: "Career" }
  ];

  const MODULES = [
    { num: "01", title: "Video Editing Fundamentals", desc: "Understanding shots, sequences, storytelling & editing workflow.", tags: ["Shots", "Storytelling"] },
    { num: "02", title: "Adobe Premiere Pro", desc: "Timeline editing, cuts, transitions, effects & exports.", tags: ["Timeline", "Exports"] },
    { num: "03", title: "Audio Editing & Sound Design", desc: "Voice sync, background music, sound effects & audio balancing.", tags: ["Audio", "Sound Design"] },
    { num: "04", title: "Color Correction & Grading", desc: "Enhancing visuals, mood creation & cinematic looks.", tags: ["Color", "Grading"] },
    { num: "05", title: "Motion Graphics (After Effects)", desc: "Text animations, lower thirds & basic visual effects.", tags: ["Motion", "VFX"] },
    { num: "06", title: "Social Media & Digital Video Content", desc: "Reels, shorts, YouTube videos & ad creatives.", tags: ["Reels", "Shorts"] },
    { num: "07", title: "Advanced Editing Techniques", desc: "Green screen, masking, transitions & visual effects.", tags: ["Green Screen", "Masking"] },
    { num: "08", title: "Portfolio & Live Projects", desc: "Real-world projects, showreel creation & presentation.", tags: ["Showreel", "Portfolio"] }
  ];
  ...
  const WHO_CAN_JOIN = [
    ...
    "Anyone who wants to become a Video Editor",
    "Those looking for video editing course near me"
  ];

  const CAREERS = [
    { icon: Film, title: "Video Editor", desc: "Work as a professional video editor in production houses, media companies, and agencies." },
    { icon: Youtube, title: "Content Creator", desc: "Create engaging video content for YouTube, social media, and digital platforms." },
    { icon: Globe2, title: "Freelance Editor", desc: "Work independently and build your own video editing brand with a strong showreel." }
  ];

  const WHY_LEARN = [
    "High demand for skilled video editors",
    "Work in diverse industries — media, advertising, entertainment",
    "Build a creative career with unlimited growth",
    "Freelance opportunities with global clients",
    "Express your creativity and build a strong showreel",
    "Join the best video editing institute in Bhopal"
  ];

  const FAQS = [
    { q: "Who can join this course?", a: "Beginners, students, and content creators are welcome. No prior editing experience required." },
    { q: "Do I need prior video editing experience?", a: "No, basic computer knowledge is sufficient. We teach everything from fundamentals to advanced editing." },
    ...
    { q: "Is placement support available?", a: "Yes, we provide resume building, showreel creation, and placement assistance." }
  ];
  ...
  return (
    <div className="bg-ink text-paper font-body min-h-screen overflow-x-hidden relative selection:bg-brand selection:text-paper">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-8 md:px-14 pt-28 md:pt-32 pb-28">
        ...
        {/* ===== HERO: EDITING TIMELINE MOCKUP ===== */}
        <section id="hero-builder" className="reveal mb-6">
          <div className="bg-surface border border-white/10 rounded-xl overflow-hidden">
            {/* editor toolbar */}
            <div className="flex items-center justify-between px-4 py-3 border-b border-white/5">
              <div className="flex items-center gap-3">
                <div className="w-6 h-6 rounded-full bg-brand/15 border border-brand/30 flex items-center justify-center text-brand font-black text-[10px]">
              ...
              <div className="flex items-center gap-1.5 text-[10px] font-mono uppercase tracking-widest">
                <span className={`w-1.5 h-1.5 rounded-full ${exported ? 'bg-brand publish-dot' : 'bg-white/20'}`} />
                <span className={exported ? 'text-brand font-bold' : 'text-muted'}>{exported ? 'Exported' : 'Rendering…'}</span>
              </div>
            </div>
            ...
            <div className="px-5 md:px-7 pb-6 md:pb-8 pt-4 border-t border-white/5">
              <div className="flex items-center gap-2 mb-3 text-[10px] font-mono uppercase tracking-widest text-muted">
                <span className="text-brand font-bold">Premiere Pro</span>•<span className="text-brand font-bold">After Effects</span>•<span>Color Grading</span>
              </div>
              <h1 className="text-3xl md:text-5xl font-black text-paper uppercase tracking-tight leading-[1.05] mb-4">
                Video <span className="text-brand">Editing</span> Course
              </h1>
              <p className="text-muted text-sm md:text-base leading-relaxed max-w-2xl mb-3">
                IADE offers a practical and industry-oriented Video Editing Course for aspiring editors
                and content creators. Recognized as one of the best video editing institutes in Bhopal,
                we focus on creative storytelling, technical editing skills, and hands-on training with
                industry tools.
              </p>
              <p className="text-muted text-sm md:text-base leading-relaxed max-w-2xl mb-6">
                This course is ideal for students, YouTubers, digital marketers, filmmakers, and
                professionals who want to build a strong career in video editing and post-production.
              </p>
        ...
        <section className="reveal mb-24">
          ...
          <h2 className="text-2xl md:text-3xl font-black text-paper uppercase tracking-tight mb-2">
            Why Choose Our <span className="text-brand">Video Editing Course</span> in Bhopal
          </h2>
        ...
        <section className="reveal mb-24">
          ...
          <p className="text-muted text-sm mb-8">A career-focused curriculum designed to make you industry-ready.</p>
        ...
        <section className="reveal mb-24">
          ...
          <h2 className="text-2xl md:text-3xl font-black text-paper uppercase tracking-tight mb-2">
            Who Can Join This <span className="text-brand">Video Editing Course</span>
          </h2>
          <p className="text-muted text-sm mb-8">No prior editing experience needed — if you're creative and eager to learn, this course is for you.</p>
        ...
        <section className="reveal mb-24">
          ...
          <p className="text-muted text-sm mb-8">After completing our video editing course in Bhopal, you can build a successful career in:</p>
          <div className="grid md:grid-cols-3 gap-4">
            {CAREERS.map((c, i) => (
              <div key={i} className="ad-card bg-surface border border-white/5 rounded-xl p-6 text-brand">
        ...
        <section className="reveal mb-24">
          ...
          <h2 className="text-2xl md:text-3xl font-black text-paper uppercase tracking-tight mb-8">
            Why Learn <span className="text-brand">Video Editing</span>
          </h2>
          <div className="grid md:grid-cols-2 gap-3">
        ...
        <section className="reveal relative overflow-hidden rounded-2xl border border-brand/30 bg-surface p-8 md:p-10">
          ...
          <div className="relative z-10">
            <div className="flex items-center gap-2 text-[10px] font-mono uppercase tracking-widest text-muted mb-4">
              <Film className="w-3.5 h-3.5 text-brand" /> Enrollment Open
            </div>
            <h2 className="text-2xl md:text-4xl font-black text-paper uppercase tracking-tight mb-3">
              Launch Your <span className="text-brand">Video Editing Career</span>
            </h2>
            <p className="text-muted text-sm max-w-md mb-6">
              Join Bhopal's best video editing institute and start building your showreel.
            </p>
